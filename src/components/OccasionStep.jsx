...
import { OCCASIONS } from '../constants/enums';

const OccasionStep = ({ selected, onSelect, onNext, onBack }) => {
  return (
      ...
      <div className="max-w-5xl w-full luxury-card rounded-3xl shadow-luxury p-8 md:p-12 relative z-10">
        <div className="mb-10 text-center">
          ...
          <h2 className="font-serif text-4xl md:text-5xl font-bold text-white mb-4 tracking-wide animate-fade-in-up">
            Qual é a <span className="gold-text">Ocasião</span>?
          </h2>
          <p className="text-gray-400 text-lg animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            O contexto social define a elegância da escolha
          </p>
          ...
          {OCCASIONS.map((occasion, index) => (
            ...
            >
              <button
                ...
                className={`selection-card w-full group p-6 rounded-2xl transition-all duration-300 ${
                  selected === occasion.value ? 'selected' : ''
                }`}
              >
                <div className="flex flex-col items-center text-center gap-3">
                  ...
                  {selected === occasion.value && (
                    <div className="mt-2 w-2 h-2 rounded-full bg-gold-500 animate-pulse shadow-gold"></div>
                  )}
                </div>
              </button>
            </div>
